...
import { Division, Game, Stats, Team, TeamFilters } from './data.models';
...
export class NbaService {
  ...
  getTeamsByFiltered({ teams, con, div }: TeamFilters): Team[] {
    return teams.filter(({ conference, division }) => {
      const isFitCon = !con || conference === con;
      const isFitDiv = !div || division === div;
      return isFitCon && isFitDiv;
    });
  }
  ...
  getDivisionFiltered({
    ...
    return divisions.filter((d) => !conference || d.con === conference);
  }
